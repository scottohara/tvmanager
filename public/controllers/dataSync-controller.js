...
define(
	...
	/**
	 * @exports controllers/dataSync-controller
	 */
	(Setting, Sync, ApplicationController, Episode, Program, Series, hexMD5, $) => {
		"use strict";
		...
		// Get a reference to the application controller singleton, and populate a hash of all models so that they can be referenced later dynamically by name
		const appController = new ApplicationController(),
					models = {Episode, Program, Series};
					...
					$.ajax({
						...
						success: (importObj, status, jqXHR) => {
							...
							const {importJson, returnedHash} = this.getImportData(importObj, jqXHR),
										hash = hexMD5(JSON.stringify(importJson));
