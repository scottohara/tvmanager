...
define(
	 ...
	 */
	function(Setting, Sync, ApplicationController, Episode, Program, Series, hex_md5, $) {
		"use strict";
		 ...
		 * @constructor DataSyncController
		 */
		...
		DataSyncController.prototype.sendChange = function(sync) {
			...
			models[sync.type].find(sync.id, $.proxy(function(instance) {
						...
						if (hash === returnedHash) {
							...
							if (importObj.length > 0) {
								...
								for (var i = 0; i < importObj.length; i++) {
									...
									obj = models[importObj[i].doc.type].fromJson(importObj[i].doc);
