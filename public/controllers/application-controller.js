...
define(
	 ...
	 */
	(require, TouchEventProxy, CacheController, DatabaseController, SpinningWheel, Abc, $) => {
		"use strict";
		...
		class ApplicationController {
					...
					this.db = new DatabaseController(dbConfig.databaseName, version => {
						...
						if (version.initial === version.current) {
							...
							this.loadDependencies(
								...
								(AboutController, DataSyncController, EpisodeController, EpisodesController, ProgramController, ProgramsController, RegistrationController, ReportController, ScheduleController, SeriesController, SeriesListController, SettingsController, UnscheduledController) => {
			...
			gotAppConfig(config, status, jqXHR) {
				...
				this.loadDependencies(["models/setting-model"], Setting => Setting.get("LastSyncTime", this.gotLastSyncTime.bind(this)));
			}
