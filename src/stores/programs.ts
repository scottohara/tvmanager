...
function create(db: IDBPDatabase<TVManagerDB>): ProgramsStore {
	return {
		async list(): Promise<PersistedProgram[]> {
			const tx = db.transaction(["programs", "series", "episodes"]),
				...
				programs = await Promise.all(
					(await txProgramsStore.index("name").getAll()).map(
						async ({
							id,
							name,
						}: ProgramsStoreObject): Promise<PersistedProgram> => {
							const series = await Promise.all(
									(await txSeriesStore.index("programId").getAllKeys(id)).map(
										async (seriesId: string): Promise<EpisodeCounts> => ({
								...
								),
								{
									...
									recordedCount,
									expectedCount,
								}: ProgramEpisodeCounts = series.reduce(
									(
										counts: ProgramEpisodeCounts,
										seriesCounts: EpisodeCounts,
									): ProgramEpisodeCounts => {
										counts.seriesCount++;
		...
		async find(id): Promise<PersistedProgram | undefined> {
			const { id: ProgramID, name: Name } = (await db.get(
				"programs",
				id,
		...
		async save({ ProgramID, Name }: PersistedProgram): Promise<void> {
			...
			await Promise.all([
				...
				tx
					.objectStore("syncs")
					.put({ type: "Program", id: ProgramID, action: "modified" }),
			]);
		...
		async remove(id: string): Promise<void> {
			const tx = db.transaction(
					["programs", "series", "episodes", "syncs"],
					"readwrite",
				),
				txProgramsStore = tx.objectStore("programs"),
				txSeriesStore = tx.objectStore("series"),
			...
			for (const seriesId of await txSeriesStore
				.index("programId")
				.getAllKeys(id)) {
				episodeIds.push(txEpisodesStore.index("seriesId").getAllKeys(seriesId));
				operations.push(
					txSyncStore.put({ type: "Series", id: seriesId, action: "deleted" }),
				);
			...
			operations.push(
				txSyncStore.put({ type: "Program", id, action: "deleted" }),
			);
