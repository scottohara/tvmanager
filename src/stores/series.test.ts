...
describe("series", (): void => {
	...
	describe("upgradeTo", (): void => {
		describe("version 1", (): void => {
			it("should create the series store", (): Chai.Assertion => expect([...db.objectStoreNames].includes("series")).to.be.true);
		});
	});
	...
	describe("create", (): void => {
		...
		describe("listByProgram", (): void => {
			it("should return a list of all series for a program", async (): Promise<Chai.Assertion> => expect(await seriesStore.listByProgram("1")).to.deep.equal([
		...
		describe("listByNowShowing", (): void => {
			it("should return a list of all series currently showing, recorded or expected", async (): Promise<Chai.Assertion> => expect(await seriesStore.listByNowShowing()).to.deep.equal([
		...
		describe("listByStatus", (): void => {
			it("should return a list of all series with episodes in a given status", async (): Promise<Chai.Assertion> => expect(await seriesStore.listByStatus("Watched")).to.deep.equal([
		...
		describe("listByIncomplete", (): void => {
			it("should return a list of all incomplete series", async (): Promise<Chai.Assertion> => expect(await seriesStore.listByIncomplete()).to.deep.equal([
		...
		describe("find", (): void => {
			it("should return a series by id", async (): Promise<Chai.Assertion> => expect(await seriesStore.find("1") as PersistedSeries).to.deep.equal({ SeriesID: "1", Name: "bb Series 1", NowShowing: null, ProgramID: "1" }));
		});

		describe("count", (): void => {
			it("should return the total number of series", async (): Promise<Chai.Assertion> => expect(await seriesStore.count()).to.equal(7));
		});

		describe("removeAll", (): void => {
			beforeEach(async (): Promise<void> => seriesStore.removeAll());
			it("should remove all series", async (): Promise<Chai.Assertion> => expect(await db.count("series")).to.equal(0));
		});
		...
		describe("save", (): void => {
			...
			it("should save the series", async (): Promise<Chai.Assertion> => expect(await db.get("series", id) as SeriesStoreObject).to.deep.equal({ id, name, nowShowing, programId }));
			it("should add a sync record", async (): Promise<Chai.Assertion> => expect(await db.get("syncs", ["Series", id]) as SyncsStoreObject).to.deep.equal({ type: "Series", id, action: "modified" }));
		});
		...
		describe("remove", (): void => {
			...
			it("should remove the series", async (): Promise<Chai.Assertion> => expect(await db.get("series", id)).to.be.undefined);
			it("should add a sync record for the series", async (): Promise<Chai.Assertion> => expect(await db.get("syncs", ["Series", id]) as SyncsStoreObject).to.deep.equal({ type: "Series", id, action: "deleted" }));
			it("should remove all episodes for the series", async (): Promise<Chai.Assertion[]> => Promise.all(episodeIds.map(async (episodeId: string): Promise<Chai.Assertion> => expect(await db.get("episodes", episodeId)).to.be.undefined)));
