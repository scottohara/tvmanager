...
import { create, upgradeTo } from "./settings";
...
describe("settings", (): void => {
	...
	describe("upgradeTo", (): void => {
		describe("version 1", (): void => {
			it("should create the settings store", (): Chai.Assertion =>
				expect([...db.objectStoreNames].includes("settings")).to.be.true);
		});
	});
	...
	describe("create", (): void => {
		...
		describe("get", (): void => {
			beforeEach(
				async (): Promise<string> => db.put("settings", { name, value }),
			);
			it("should return a setting by name", async (): Promise<Chai.Assertion> =>
				expect(
					(await settingsStore.get(name)) as PersistedSetting,
				).to.deep.equal({ name, value }));
		});

		describe("save", (): void => {
			beforeEach(async (): Promise<string> => settingsStore.save(name, value));
			it("should save the setting", async (): Promise<Chai.Assertion> =>
				expect(
					(await db.get("settings", name)) as PersistedSetting,
				).to.deep.equal({ name, value }));
		});
		...
		describe("remove", (): void => {
			...
			it("should remove a setting by name", async (): Promise<Chai.Assertion> =>
				expect(await db.get("settings", name)).to.be.undefined);
		});
	});
